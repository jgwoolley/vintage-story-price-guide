...
export default function PathStepsTable({pathSteps, onZoomNode, sourceNode, destinationNode}: PathStepsTableProps) {
    if(sourceNode == undefined || destinationNode == undefined || pathSteps.length < 2) {
        return null;
    }
    ...
    return (
        <TableContainer>
            <Table>
                ...
                <TableBody>
                    <TableRow>
                        <TableCell sx={{ cursor: "pointer"}} onClick={() => onZoomNode(pathSteps[0].from)}>{pathSteps[0].from.data("label")}</TableCell>
